...
class Display(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if (len(self.marked) >= 2):
            ...
            for i in range(0, len(self.marked), 2):
                ...
                if (i + 1 < len(self.marked) and (
                        self.points[self.marked[i]][1] == True and self.points[self.marked[i + 1]][1])):
                    triangle_source = calculate_arrow_points(self.points[self.marked[i]][0],
        ...
        if (self.functionAble == "Добавить вершину"):
            ...
            for i in range(self.cntPoints):
                if ((event.button() == Qt.LeftButton) and
                        (event.pos().x() >= self.points[i][0].x() - self.r and event.pos().x() <= self.points[i][
                            0].x() + self.r and
                         event.pos().y() >= self.points[i][0].y() - self.r and event.pos().y() <= self.points[i][
                             0].y() + self.r)):
            ...
            for i in range(self.cntPoints):
                ...
                if ((event.pos().x() >= self.points[i][0].x() - sensitivity and event.pos().x() <= self.points[i][
                    0].x() + sensitivity and
                     event.pos().y() >= self.points[i][0].y() - sensitivity and event.pos().y() <= self.points[i][
                         0].y() + sensitivity)):
    ...
    def mouseMoveEvent(self, event):
        ...
        if (self.functionAble == "Переместить вершины"):
            for i in range(self.cntPoints):
                if (event.pos().x() >= self.points[i][0].x() - self.r and event.pos().x() <= self.points[i][
                    0].x() + self.r and
                        event.pos().y() >= self.points[i][0].y() - self.r and event.pos().y() <= self.points[i][
                            0].y() + self.r):
                    self.points[i][0] = event.pos()
