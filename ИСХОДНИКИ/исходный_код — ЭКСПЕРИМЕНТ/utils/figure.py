...
class Figure:
    ...
    def draw_links(self, painter: QPainter):
        for other_figure in self.links.values():
            painter.drawLine(int(self.vertex[0] + self.w / 2), int(self.vertex[1] + self.h / 2), int(other_figure.vertex[0] + other_figure.w / 2), int(other_figure.vertex[1] + other_figure.h / 2))