...
class Checker:
    ...
    def _doIntersect(p1, q1, p2, q2):
        o1 = Checker._orientation(p1, q1, p2)
        o2 = Checker._orientation(p1, q1, q2)
        o3 = Checker._orientation(p2, q2, p1)
        o4 = Checker._orientation(p2, q2, q1)

        if ((o1 != o2) and (o3 != o4)):
            return True

        if ((o1 == 0) and Checker._onSegment(p1, p2, q1)):
            return True
        if ((o2 == 0) and Checker._onSegment(p1, q2, q1)):
            return True
        if ((o3 == 0) and Checker._onSegment(p2, p1, q2)):
            return True
        if ((o4 == 0) and Checker._onSegment(p2, q1, q2)):
            return True
    ...
    @staticmethod
    def _isPointOnSegment(a, p, b, EPS=0.1) -> bool:
        AB = np.sqrt((b.x() - a.x()) * (b.x() - a.x()) +
                     (b.y()-a.y()) * (b.y() - a.y()))
        AP = np.sqrt((p.x() - a.x()) * (p.x() - a.x()) +
                     (p.y()-a.y()) * (p.y() - a.y()))
        PB = np.sqrt((b.x() - p.x()) * (b.x() - p.x()) +
                     (b.y()-p.y()) * (b.y() - p.y()))
    ...
    @staticmethod
    def findLatePeriods(graph, early, n):
    ...
    @staticmethod
    def findReserve(reserve, early, late, n):
    ...
    @staticmethod
    def _topologicalSort(v):
        visited[v] = True

        for i in adj[v]:
            if (not visited[i[0]]):
                Checker._topologicalSort(i[0])

        Stack.append(v)

    @staticmethod
    def _criticalPath(s, n):
        dist = [-np.inf for _ in range(n)]

        for i in range(n):
            if (visited[i] == False):
                Checker._topologicalSort(i)
        ...
        if u == d:
            ...
            for i in adj[u]:
                if visited[i[0]] == False:
                    Checker._findAllPaths(i[0], d, path)

        path.pop()
        visited[u] = False

    @staticmethod
    def checkIntersections(mistakes, mistake_id, Points, AdjacencyMatrix):
        # (i, j) - first edge
                    ...
                    for k, row2 in enumerate(AdjacencyMatrix):
                        for l, col2 in enumerate(row2):
                            ...
                            if col2 >= 1:
                                ...
                                # q1 == q2
                                if (j == l):
                                    if (Checker._isPointOnSegment(p1, p2, q1) or Checker._isPointOnSegment(p2, p1, q1)):
                                        mistakes.append(mistake_id)
                                        return mistakes
                                # p1 == p2
                                elif (i == k):
                                    if (Checker._isPointOnSegment(p1, q1, q2) or Checker._isPointOnSegment(p1, q2, q1)):
                                        mistakes.append(mistake_id)
                                        return mistakes
                                # q1 == p2
                                elif (j == k):
                                    if (Checker._isPointOnSegment(p1, q2, q1) or Checker._isPointOnSegment(q2, p1, q1)):
                                        mistakes.append(mistake_id)
                                        return mistakes
                                # q2 == p1
                                elif (i == l):
                                    if (Checker._isPointOnSegment(p2, q1, p1) or Checker._isPointOnSegment(q1, p2, p1)):
                                        mistakes.append(mistake_id)
                                        return mistakes

                                elif (Checker._doIntersect(p1, q1, p2, q2)):
                                    mistakes.append(mistake_id)
                                    return mistakes
        return mistakes

    @staticmethod
    def checkConnections(mistakes, mistake_id, AdjacencyMatrix, CorrectAdjacencyMatrix):
    ...
    if (not ignore):
        ...
        if CorrectCountOfConnections != CurrentCountOfConnections:
            ...
            Checker.checkConnections(
                mistakes, TaskOneMistakes.WRONG_CONNECTIONS.value, Graph.AdjacencyMatrix, CorrectAdjacencyMatrix)

        # проверим на пересечение рёбер
        Checker.checkIntersections(
            mistakes, TaskOneMistakes.INTERCETING_CONNECTIONS.value, Graph.Points, Graph.AdjacencyMatrix)
    ...
    global Stack, visited, adj, paths
        ...
        correct_max_distance = Checker._criticalPath(0, n)
        ...
        Checker._findAllPaths(0, n-1, path)
        ...
        max_distance = Checker._criticalPath(0, n)
    ...
    if (old_mistakes):
        ...
        for i in range(n):
            Graph.PointsTimeEarly[i] = round(
                (Graph.Points[i][0] - GridBegin) / GridStep) - Indent

        points_on_correct_axes = True
        for i in range(n):
            if (Graph.PointsTimeEarly[i] != early[i]):
                mistakes.append(
                    TaskThreeAndFourMistakes.POINTS_ON_WRONG_TIME_POSITIONS.value)
                mistakes.append(
            ...
            for j in range(len(CorrectAdjacencyMatrix)):
                if (CorrectAdjacencyMatrix[i][j] == 1):
                    Graph.ArrowPointsTimeEarly[i][j] = round(
                        (Graph.ArrowPoints[i][j][0] - GridBegin) / GridStep) - Indent
                else:
    ...
    @staticmethod
    def _isSubstring(word_list, text_list):
    ...
    def checkTask5Part1(Graph, BaseGraph, Id) -> bool:
        ...
        AdjacencyMatrix = [([0] * len(CorrectAdjacencyMatrix))
                           for _ in range(len(CorrectAdjacencyMatrix))]
        ...
        mistakes = []
        mistake_id = 0
        Checker.checkConnections(mistakes, mistake_id,
                                 AdjacencyMatrix, CorrectAdjacencyMatrix)
        ...
        for i, s1 in enumerate(Graph.Sequences):
            for j, s2 in enumerate(Graph.Sequences):
                if (i != j) and Checker._isSubstring(s1, s2):
                    is_correct = False
                    return is_correct
    ...
    def checkTask5Part2(Graph, BaseGraph, CorrectWeights, GridBegin, GridStep, Id) -> bool:
        ...
        for digit in Points:
            for p in Graph.Points.keys():
                if p[0] == digit:
                    (x, y) = Graph.Points[p]
                    Graph.PointsTimeEarly[digit] = round(
                        (x - GridBegin) / GridStep) - Indent
                    break
        ...
        for (digit1, id1), (digit2, id2) in Graph.AdjacencyList.items():
            ...
            if (CorrectAdjacencyMatrix[digit1][digit2] == 1):
                Graph.ArrowPointsTime[digit1][digit2] = round(
                    (x - GridBegin) / GridStep) - Indent
            else:
