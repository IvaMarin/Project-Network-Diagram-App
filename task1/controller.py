# Controller составл€юща€ MVC (√раф)
...
# св€зать вершины по нажатию; параметры: объект "граф", событие, кнопка
def CConnectPoints(graph, event, but):
	# если нажата кнопка
	if event.button() == but:
		graph.ConnectPoints(graph.IsCursorOnPoint(event.pos().x(), event.pos().y())) # св€зать вершины

# удалить св€зь между вершинами по нажатию; параметры: объект "граф", событие, кнопка
def CDeleteConnection(graph, event, but):
	# если нажата кнопка
	if event.button() == but:
		graph.DeleteConnection(graph.IsCursorOnPoint(event.pos().x(), event.pos().y()),
									  graph.IsCursorOnPoint(event.pos().x(), event.pos().y())) # удалить св€зь между вершинами
